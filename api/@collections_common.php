...
function collectionsCommon_checkRequestTextData($data)
{
    if (
        ...
        $data["status"] < 1 ||
        ($data["type"] < 3 &&
            (!isset($data["animal_id"]) || $data["animal_id"] < 1)) ||
        !isset($data["target_sum"]) ||
        !$data["target_sum"]
    ) {
...
function collectionsCommon_updateCollectionStatus($id)
{
    ...
    if (
        $_row["status"] === "1" &&
        ($_row["target_sum"] === "0" ||
            floatval($_row["target_sum"]) <= floatval($_row["collected"]))
    ) {
        $_status = "3";
    } elseif (
        $_row["status"] === "3" &&
        floatval($_row["collected"]) > 0 &&
        floatval($_row["target_sum"]) > floatval($_row["collected"])
    ) {
        $_status = "1";
    }
    ...
    if ($_status) {
        $db_mysqli->query(
            "UPDATE collections SET status = $_status WHERE id=$id"
        );
    }
}
?>
