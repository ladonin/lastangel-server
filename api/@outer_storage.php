...
function outerStorage_headersToArray($str)
{
    ...
    for ($_i = 0; $_i < count($_headersTmpArray); ++$_i) {
        if (
            strlen($_headersTmpArray[$_i]) > 0 &&
            strpos($_headersTmpArray[$_i], ":")
        ) {
            $_headerName = substr(
                $_headersTmpArray[$_i],
                0,
...
$outerStorage_response = shell_exec(
    "curl -i -XGET https://api.selcdn.ru/auth/v1.0 -H 'X-Auth-User: $CONFIGS_OUTER_STORAGE_X_AUTH_USER' -H 'X-Auth-Key: $CONFIGS_OUTER_STORAGE_X_AUTH_KEY'"
);
...
function outerStorage_uploadFile($src, $folder)
{
    ...
    exec(
        "curl -i -XPUT ${outerStorage_url}${OUTER_STORAGE_URL}${folder}/ -H 'X-Auth-Token: ${outerStorage_authToken}'  -T $src",
        $_output
    );
    if (!count($_output) || strpos($_output[0], " 201") === false) {
        functions_errorOutput(
            "ошибка загрузки файла на внешний сервер: " . $src,
            500
        );
    }
}
...
function outerStorage_removeFile($file, $folder)
{
    global $outerStorage_url, $outerStorage_authToken, $OUTER_STORAGE_URL;
    exec(
        "curl -i -XDELETE ${outerStorage_url}${OUTER_STORAGE_URL}${folder}/${file} -H 'X-Auth-Token: ${outerStorage_authToken}'",
        $_output
    );

    if (!count($_output) || strpos($_output[0], " 204") === false) {
        functions_errorOutput(
            "ошибка удаления файла c внешнего сервера: " .
                $folder .
                "/" .
                $file,
            500
...
function outerStorage_removeFolder($folder)
{
    global $outerStorage_url, $outerStorage_authToken, $OUTER_STORAGE_URL;
    exec(
        "curl -i -XDELETE ${outerStorage_url}${OUTER_STORAGE_URL}${folder} -H 'X-Auth-Token: ${outerStorage_authToken}'",
        $_output
    );
    if (!count($_output) || strpos($_output[0], " 204") === false) {
