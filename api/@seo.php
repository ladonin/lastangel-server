...
';
    ...
    while ($_row = $_res->fetch_assoc()) {
        $_sitemapText .=
            '	<url>
		<loc>https://lastangel.ru/' .
            prepareSinglePageName($section) .
            "/" .
            $_row["id"] .
            '</loc>
		<changefreq>monthly</changefreq>
...
';
    ...
    $_res = $db_mysqli->query(
        "SELECT * from " .
            prepareDBName($section) .
            " ORDER by id DESC LIMIT 10000 OFFSET 10"
    );
    $_pets = [];
...
';
    ...
    foreach ($_sections as $_section => $_time) {
        $_sitemapText .=
            '	<sitemap>
		<loc>https://lastangel.ru/sitemap-' .
            $_section .
            '.xml</loc>
	</sitemap>
';
