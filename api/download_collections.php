...
function convertType($val) {
	 if ($val===1) return 'нужна медпомощь';
	 if ($val===2) return 'покупка вещи для питомца';
	 if ($val===3) return 'постройка';
	 if ($val===4) return 'общие нужды';
}
function convertStatus($val) {
	 if ($val===1) return 'опубликован';
	 if ($val===2) return 'не опубликован';
	 if ($val===3) return 'закрыт';
}
...
if ($_type === 'csv') {
	$_data.="id;Наименование;Тип;Статус;Кому;Краткое описание;Полное описание;Надо собрать;Дата создания;Дата последнего обновления\n";
}
if ($_type === 'html') {
...
while($_row = $_result->fetch_array()){
	if ($_type === 'txt') {
		...
		$_data.="#Кому: ". ($_row['animal_id'] ? $_row['animal_id'] : '-')."\n";
		$_data.="#Краткое описание: ". trim($_row['short_description'])."\n";
	...
	if ($_type === 'html') {
		...
		$_data.="<b>Кому</b>: ".($_row['animal_id'] ? $_row['animal_id'] : '-')."<br/>";
		$_data.="<b>Краткое описание</b>: ".trim($_row['short_description'])."<br/>";
