...
$_stmt = $db_mysqli->prepare("SELECT * FROM stories order by id DESC");
$_stmt->execute();
...
while ($_row = $_result->fetch_array()) {
    ...
    if ($_type === "html") {
        ...
        $_data .=
            "<b>Краткое описание</b>: " . $_row["short_description"] . "<br/>";
        $_data .= "<b>Описание</b>: " . $_row["description"] . "<br/>";
        $_data .=
            "<b>Описание (мобильная версия)</b>: " .
            $_row["mobile_description"] .
            "<br/>";
...
header("Content-Disposition: attachment; filename=stories." . $_type);
