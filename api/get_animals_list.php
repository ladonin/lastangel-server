...
$_sql =
    "SELECT
		...
		animals.created,
		animals.updated,
		donations.sum as collected,
		collections.need_medicine as need_medicine
	FROM `animals`
	LEFT JOIN (SELECT SUM(sum) as sum, target_id, created FROM `donations` WHERE type= 1 AND created >= '" .
		$_limitTimeCondition .
		"' GROUP BY target_id) donations
		ON donations.target_id = animals.id
	LEFT JOIN (SELECT count(*) as need_medicine, animal_id FROM `collections` WHERE type= 1 AND status=1 GROUP BY animal_id) collections
		ON collections.animal_id = animals.id
	WHERE 1 ";
