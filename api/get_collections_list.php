...
$_sql =
    "SELECT
		...
		END AS is_corrupted
	FROM `collections`
	LEFT JOIN (SELECT SUM(sum) as sum, target_id, type FROM `donations` WHERE type=2  GROUP BY target_id) donations
		ON donations.target_id = collections.id
	LEFT JOIN animals
		ON collections.animal_id = animals.id AND (collections.type=1 OR collections.type=2)
	WHERE NOT (" .
		($_with_corrupted ? "0" : "1") .
		" AND animals.id IS NULL AND (collections.type = 1 OR collections.type = 2)) ";
...
} elseif (isset($_GET["withClosedCollections"])) {
...
if (isset($_GET["orderComplex"]) && $_GET["orderComplex"]) {
    ...
    if ($_orderComplex === "ismajor desc, status asc") {
