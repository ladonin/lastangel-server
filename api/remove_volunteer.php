...
$_res = $db_mysqli->query("
	SELECT
		main_image, another_images, video1, video2, video3
	FROM volunteers
	WHERE id=$_recordId"
);
...
if ($_main_image === "1") {
    $_filesSizeNames = array_unique(
        array_merge(
            images_getFileSizeNames("main", "", $IMAGES_MAIN_SIZES),
            images_getFileSizeNames("main", "", $IMAGES_MAIN_SQUARE_SIZES)
        )
    );
