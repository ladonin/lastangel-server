...
function processVideo($name)
{
    ...
    if (isset($_FILES[$name]) && $_FILES[$name]) {
        ...
        $_res = $db_mysqli->query(
            "SELECT $name FROM acquaintanceship WHERE id=1"
        );
        ...
        if (move_uploaded_file($_FILES[$name]["tmp_name"], $_pathVideo)) {
            ...
            $db_mysqli->query(
                "UPDATE acquaintanceship SET $name = '$_videoFileName' WHERE id = 1"
            );
        } else {
        ...
        if ($_oldVideo) {
            ...
            outerStorage_removeFile($_oldVideo, "acquaintanceship");
            $db_mysqli->query(
                "UPDATE acquaintanceship SET $name = '' WHERE id = 1"
            );
        }
    }
...
$_res = $db_mysqli->query(
    "SELECT another_images FROM acquaintanceship WHERE id=1"
);
$_row = $_res->fetch_assoc();
...
$db_mysqli->query(
    "UPDATE acquaintanceship SET another_images = '" .
        $_another_photosJSON .
        "' WHERE id = 1"
);
///////////////////// <-- ФОТО В БД
