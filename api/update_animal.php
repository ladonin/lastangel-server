...
function processVideo($name)
{
    ...
    if (isset($_FILES[$name]) && $_FILES[$name]) {
        ...
        $_res = $db_mysqli->query(
            "SELECT $name FROM animals WHERE id='$_recordId'"
        );
        ...
        if (move_uploaded_file($_FILES[$name]["tmp_name"], $_pathVideo)) {
            ...
            $db_mysqli->query(
                "UPDATE animals SET $name = '$_videoFileName' WHERE id = '$_recordId'"
            );
        } else {
        ...
        if ($_oldVideo) {
            ...
            outerStorage_removeFile($_oldVideo, "pets/" . $_recordId);
            $db_mysqli->query(
                "UPDATE animals SET $name = '' WHERE id = '$_recordId'"
            );
        }
    }
...
$_res = $db_mysqli->query(
    "SELECT main_image, another_images FROM animals WHERE id=$_recordId"
);
$_row = $_res->fetch_assoc();
...
$db_mysqli->query(
    "UPDATE animals SET another_images = '" .
        $_another_photosJSON .
        "', main_image='" .
