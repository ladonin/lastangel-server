...
$_target_sum = trim(str_replace(",", ".", $_data["target_sum"]));
$_current_spending = isset($_data["current_spending"])
    ? strval(floatval($_data["current_spending"]))
    : "0";
$_stmt->bind_param(
    "siiississsi",
    $_data["name"],
...
// --> Обновляем снимок названия для возможных донатов на данный сбор
// Если его удалят, то показывать последнее актуальное название
$_stmt = $db_mysqli->prepare("UPDATE donations
SET target_print_name=? WHERE type=2 AND target_id=$_recordId");
...
function processVideo($name)
{
    ...
    if (isset($_FILES[$name]) && $_FILES[$name]) {
        ...
        $_res = $db_mysqli->query(
            "SELECT $name FROM collections WHERE id='$_recordId'"
        );
        ...
        if (move_uploaded_file($_FILES[$name]["tmp_name"], $_pathVideo)) {
            ...
            $_oldVideo &&
                outerStorage_removeFile(
                    $_oldVideo,
                    "collections/" . $_recordId
                );
            outerStorage_uploadFile($_pathVideo, "collections/" . $_recordId);
            $db_mysqli->query(
                "UPDATE collections SET $name = '$_videoFileName' WHERE id = '$_recordId'"
            );
        } else {
        ...
        if ($_oldVideo) {
            ...
            $db_mysqli->query(
                "UPDATE collections SET $name = '' WHERE id = '$_recordId'"
            );
        }
    }
...
$_res = $db_mysqli->query(
    "SELECT main_image, another_images FROM collections WHERE id=$_recordId"
);
$_row = $_res->fetch_assoc();
...
$db_mysqli->query(
    "UPDATE collections SET another_images = '" .
        $_another_photosJSON .
        "', main_image='" .
