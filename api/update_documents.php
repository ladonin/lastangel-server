...
$_stmt = $db_mysqli->prepare("UPDATE documents
SET 
	updated=? 
WHERE id=1");
$_now = time();
...
///////////////////// --> ФОТО ФАЙЛЫ
$_tempFolder = $IMAGES_TEMPFOLDER_PATH.'documents/';
...
$_res = $db_mysqli->query("SELECT another_images FROM documents WHERE id=1");
...
if (isset($_data['another_images_for_delete'])) {
	...
	foreach ($_data['another_images_for_delete'] as $_number) {
		...
		foreach ($_filesSizeNames as $_fileName) {
			outerStorage_removeFile($_fileName, 'documents');
		}
	}
...
// --> Загружаем на внешнее хранилище
	foreach ($_anotherImages as $_path) {
		outerStorage_uploadFile($_tempFolder.$_path, 'documents');
	}
// <-- Загружаем на внешнее хранилище
...
$db_mysqli->query("UPDATE documents SET another_images = '". $_another_photosJSON ."' WHERE id = 1");
///////////////////// <-- ФОТО В БД
