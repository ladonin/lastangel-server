...
if ($_data["type"] === 1 || $_data["type"] === 2) {
    ...
    $_res = $db_mysqli->query(
        "SELECT name FROM " .
            ($_data["type"] === 1 ? "animals" : "collections") .
            " WHERE id=" .
            $_id
    );
...
$_donator_firstname = isset($_data["donator_firstname"])
    ? strtolower(trim($_data["donator_firstname"]))
    : "";
$_donator_middlename = isset($_data["donator_middlename"])
    ? strtolower(trim($_data["donator_middlename"]))
    : "";
$_donator_lastname = isset($_data["donator_lastname"])
    ? strtolower(trim($_data["donator_lastname"]))
    : "";
$_donator_card = isset($_data["donator_card"])
    ? strtolower(trim($_data["donator_card"]))
    : "";

$_sum = trim(str_replace(",", ".", $_data["sum"]));
...
if (
    $_oldDonationData["type"] === "2" &&
    $_newDonationData["type"] === "2" &&
    $_oldDonationData["target_id"] === $_newDonationData["target_id"]
) {
    ...
    if ($_oldDonationData["type"] === "2") {
        collectionsCommon_updateCollectionStatus(
            $_oldDonationData["target_id"]
        );
    }
    if ($_newDonationData["type"] === "2") {
        collectionsCommon_updateCollectionStatus(
            $_newDonationData["target_id"]
        );
    }
}
// <--- Возможное закрытие/открытие сбора
