...
$_stmt = $db_mysqli->prepare("
	UPDATE mainphotoalbum
	SET
		updated=?
	WHERE id=1");
$_now = time();
$_stmt->bind_param("i", $_now);
$_stmt->execute();
///////////////////// <-- ОСНОВНЫЕ ДАННЫЕ
...
$_res = $db_mysqli->query(
    "SELECT another_images FROM mainphotoalbum WHERE id=1"
);
...
$db_mysqli->query(
    "UPDATE mainphotoalbum SET another_images = '" . $_another_photosJSON . "' WHERE id = 1"
);
///////////////////// <-- ФОТО В БД
