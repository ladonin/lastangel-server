...
$_stmt = $db_mysqli->prepare("UPDATE mainphotoalbum
SET 
	updated=? 
WHERE id=1");
$_now = time();
...
///////////////////// --> ФОТО ФАЙЛЫ
$_tempFolder = $IMAGES_TEMPFOLDER_PATH.'mainphotoalbum/';
...
$_res = $db_mysqli->query("SELECT another_images FROM mainphotoalbum WHERE id=1");
...
if (isset($_data['another_images_for_delete'])) {
	...
	foreach ($_data['another_images_for_delete'] as $_number) {
		...
		foreach ($_filesSizeNames as $_fileName) {
			outerStorage_removeFile($_fileName, 'mainphotoalbum');
		}
	}
...
// --> Загружаем на внешнее хранилище
	foreach ($_anotherImages as $_path) {
		outerStorage_uploadFile($_tempFolder.$_path, 'mainphotoalbum');
	}
// <-- Загружаем на внешнее хранилище
...
$db_mysqli->query("UPDATE mainphotoalbum SET another_images = '". $_another_photosJSON ."' WHERE id = 1");
///////////////////// <-- ФОТО В БД
