...
function processVideo($name)
{
    ...
    if (isset($_FILES[$name]) && $_FILES[$name]) {
        ...
        if ($_oldVideo) {
            ...
            outerStorage_removeFile($_oldVideo, "news/" . $_recordId);
            $db_mysqli->query(
                "UPDATE news SET $name = '' WHERE id = '$_recordId'"
            );
        }
    }
...
$_res = $db_mysqli->query(
    "SELECT another_images FROM news WHERE id=$_recordId"
);
$_row = $_res->fetch_assoc();
...
$db_mysqli->query(
    "UPDATE news SET another_images = '" . $_another_photosJSON . "' WHERE id = '" . $_recordId . "'"
);
///////////////////// <-- ФОТО В БД
