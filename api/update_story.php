...
function processVideo($name)
{
    ...
    if (isset($_FILES[$name]) && $_FILES[$name]) {
        ...
        $_res = $db_mysqli->query(
            "SELECT $name FROM stories WHERE id='$_recordId'"
        );
        ...
        if (move_uploaded_file($_FILES[$name]["tmp_name"], $_pathVideo)) {
            ...
            $db_mysqli->query(
                "UPDATE stories SET $name = '$_videoFileName' WHERE id = '$_recordId'"
            );
        } else {
        ...
        if ($_oldVideo) {
            ...
            outerStorage_removeFile($_oldVideo, "stories/" . $_recordId);
            $db_mysqli->query(
                "UPDATE stories SET $name = '' WHERE id = '$_recordId'"
            );
        }
    }
...
$_res = $db_mysqli->query(
    "SELECT another_images FROM stories WHERE id=$_recordId"
);
$_row = $_res->fetch_assoc();
...
$db_mysqli->query(
    "UPDATE stories SET another_images = '" .
        $_another_photosJSON .
        "' WHERE id = '" .
        $_recordId .
        "'"
